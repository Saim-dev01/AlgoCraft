...
const PostorderTraversal = ({ root }) => {
  ...
  const generatePostorderSteps = (node, newSteps, sortedList) => {
    if (!node) return;

     // Traverse left subtree
    newSteps.push({ text: `Traversing left subtree of: ${node.name}`, node, highlight: 2 });
    generatePostorderSteps(node.children[0], newSteps, sortedList);
    // Backtrack from left
    newSteps.push({ text: `Backtracking to node: ${node.name} from left`, node, highlight: 2, backtrack: true });

    // Traverse right subtree
    newSteps.push({ text: `Traversing right subtree of: ${node.name}`, node, highlight: 3 });
    generatePostorderSteps(node.children[1], newSteps, sortedList);
    // Backtrack from right
    newSteps.push({ text: `Backtracking to node: ${node.name} from right`, node, highlight: 3, backtrack: true });
    // Visit the node
    newSteps.push({ text: `Visiting node: ${node.name}`, node, highlight: 4 });
  ...
  const handlePostorder = () => {
    ...
    let sortedList = [];
    let newSteps = [];
    generatePostorderSteps(root, newSteps, sortedList);
    ...
    <div className="bfs-container">
      ...
      >
        <button
          ...
          disabled={isRunning}
        >
          {isRunning ? "Running..." : "Start Postorder Traversal"}
        </button>
        <button
