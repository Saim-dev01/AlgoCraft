...
const FindMaxNode = ({ root }) => {
  ...
  const algorithmLines = [
    ...
    "  while (node.children && node.children[node.children.length - 1]) {",
  ...
  const handleFindMax = () => {
    ...
    while (currentNode && currentNode.children && currentNode.children[currentNode.children.length - 1]) {
      newSteps.push({
        text: `Traversing right to node: ${currentNode.children[currentNode.children.length - 1].name}`,
    ...
    newSteps.push({
      text: `Maximum node found: ${currentNode.name}`,
      node: currentNode,
      highlight: 5,
    });
  ...
  const renderTreeD3 = () => {
    ...
    node
      ...
      .attr("r", 20)
      .style("fill", (d) => {
        if (maxNode && d.data === maxNode) return "orange";
  ...
  useEffect(() => {
    renderTreeD3();
  }, [root, maxNode, currentHighlightNode]);

  return (
    <div className="bfs-container">
      <h1>Find Maximum Node in Tree</h1>
      <div
      ...
      >
        <button
          ...
          disabled={isFinding}
        >
          {isFinding ? "Finding..." : "Find Max Node"}
        </button>
        <button
      ...
      <div >
        {maxNode && currentStep === steps.length && (
          <div style={{
            textAlign: "center",
            fontSize: "2px",
            color: "red",
            ...
            <h5>Maximum Node is:{maxNode.name}</h5>
          </div>
        )}
      </div>
