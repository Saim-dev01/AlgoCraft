...
import '../../style/bfs.css';  // you can rename to dfs.css if you like

const DFSVisualization = ({ root }) => {
  ...
  const handleDFS = () => {
    ...
    const dfsHelper = (node) => {
      if (!node || visitedSet.has(node.name)) return;
      ...
      for (let child of node.children || []) {
        newSteps.push({
          text: `Traversing to child: ${child.name}`,
          node: child,
          highlight: 4,
        });
        dfsHelper(child);
        // Backtrack step after returning from child
        newSteps.push({
          text: `Backtracking to node: ${node.name} from child ${child.name}`,
          node: node,
          highlight: 4,
          backtrack: true,
        });
    ...
    <div className="bfs-container">
      ...
      >
        <button
          ...
          disabled={isRunning}
        >
          {isRunning ? "Running..." : "Start DFS Traversal"}
        </button>
        <button
