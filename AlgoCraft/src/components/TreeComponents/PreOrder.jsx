...
const PreorderTraversal = ({ root }) => {
  ...
  const generatePreorderSteps = (node, newSteps, sortedList) => {
    if (!node) return;

    // Visit current node first
    newSteps.push({ text: `Visiting node: ${node.name}`, node, highlight: 2 });
    sortedList.push(node.name);
 
    // Traverse left subtree
    newSteps.push({ text: `Traversing left subtree of: ${node.name}`, node, highlight: 3 });
    generatePreorderSteps(node.children[0], newSteps, sortedList);
    // Backtrack from left
    newSteps.push({ text: `Backtracking to node: ${node.name} from left`, node, highlight: 3, backtrack: true });

    // Traverse right subtree
    newSteps.push({ text: `Traversing right subtree of: ${node.name}`, node, highlight: 4 });
    generatePreorderSteps(node.children[1], newSteps, sortedList);
  ...
  const handlePreorder = () => {
    ...
    let sortedList = [];
    let newSteps = [];
    generatePreorderSteps(root, newSteps, sortedList);
    ...
    <div className="bfs-container">
      ...
      >
        <button
          ...
          disabled={isRunning}
        >
          {isRunning ? "Running..." : "Start Preorder Traversal"}
        </button>
        <button
